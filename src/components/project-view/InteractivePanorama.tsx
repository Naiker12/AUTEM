    ...
    <div ref={containerRef} className="absolute inset-0 cursor-grab touch-none active:cursor-grabbing" aria-label="Tour 360 del proyecto">
      ...
      {status === "ready" && <>
        <div className="absolute right-5 top-24 z-20 flex flex-col items-center gap-3 md:right-8">
          <button type="button" onClick={() => controlsRef.current?.reset()} className="relative flex size-20 items-center justify-center rounded-full border border-white/20 bg-black/45 text-white shadow-xl backdrop-blur-xl" aria-label="Centrar vista">
            <Compass className="size-9 text-accent" style={{ transform: `rotate(${-heading}deg)` }} />
          ...
          <div className="grid overflow-hidden rounded-2xl border border-white/20 bg-black/45 shadow-xl backdrop-blur-xl">
            <button type="button" onClick={() => controlsRef.current?.zoom(-8)} className="grid size-11 place-items-center border-b border-white/15 text-white hover:bg-white/15" aria-label="Acercar"><Plus size={18} /></button>
            <button type="button" onClick={() => controlsRef.current?.zoom(8)} className="grid size-11 place-items-center border-b border-white/15 text-white hover:bg-white/15" aria-label="Alejar"><Minus size={18} /></button>
            <button type="button" onClick={() => controlsRef.current?.reset()} className="grid size-11 place-items-center border-b border-white/15 text-white hover:bg-white/15" aria-label="Restablecer vista"><RotateCcw size={17} /></button>
            <button type="button" onClick={fullscreen} className="grid size-11 place-items-center text-white hover:bg-white/15" aria-label="Pantalla completa"><Expand size={17} /></button>
          </div>
        </div>
        <div className="pointer-events-none absolute bottom-6 left-1/2 z-20 hidden -translate-x-1/2 rounded-full border border-white/15 bg-black/45 px-4 py-2 text-[9px] font-bold uppercase tracking-[0.16em] text-white/70 backdrop-blur-xl md:block">Arrastra para recorrer · Rueda para acercar</div>
      </>}
    </div>
  );
}
