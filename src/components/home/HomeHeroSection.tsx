...
export default function HomeHeroSection({ visible }: HomeHeroSectionProps) {
  ...
  useEffect(() => {
    ...
    const updateProgress = () => {
      ...
      const progress = Math.min(1, Math.max(0, -bounds.top / (bounds.height * 0.72)));
      section.style.setProperty("--hero-scroll-progress", progress.toFixed(4));
      section.style.setProperty("--hero-scroll-opacity", (1 - progress * 0.72).toFixed(4));
      section.style.setProperty("--hero-scroll-y", `${(progress * -48).toFixed(2)}px`);
      section.style.setProperty("--hero-scroll-scale", (1 - progress * 0.025).toFixed(4));
    };
    const onScroll = () => {
  ...
  return (
    <section
      ref={sectionRef}
      id="top"
      className="relative overflow-hidden bg-background text-foreground md:h-[100svh] md:min-h-[760px]"
    >
      ...
      <div className="home-hero-scroll-stage relative mx-auto grid max-w-[1800px] px-6 pb-0 pt-28 md:h-full md:min-h-0 md:grid-cols-12 md:items-center md:px-10 md:pb-8 md:pt-24 lg:px-14 xl:px-20">
        <div className="relative z-30 md:col-span-6 lg:col-span-5">
          <svg
            className="home-copy-mesh pointer-events-none absolute -left-[38%] -top-[14%] z-0 h-[92%] w-[178%] overflow-visible"
            viewBox="0 0 760 430"
            fill="none"
            aria-hidden="true"
          >
          ...
          <p
            className={`${entranceClass} home-entrance--body relative z-10 mt-7 max-w-lg text-[17px] leading-7 text-muted-foreground md:mt-9 md:text-xl md:leading-9`}
          >
            Explora cada detalle del proyecto en 3D y descubre una nueva forma de invertir en
            espacios únicos.
          </p>
          <div
            className={`${entranceClass} home-entrance--action relative z-10 mt-8 flex flex-wrap items-center gap-2 sm:gap-4 md:mt-10`}
          >
            <Button
              asChild
              size="lg"
          ...
          <dl
            className={`${entranceClass} home-entrance--action relative z-10 mt-10 grid max-w-xl grid-cols-3 border-y border-border py-4 md:mt-12 md:py-5`}
          >
            {[
              ["Ubicación", "Cartagena"],
              ["Lotes", "Desde 1.080 m²"],
        ...
        <div
          className={`${visible ? "hero-visual-enter" : "opacity-0"} relative z-10 mt-6 min-h-[410px] sm:min-h-[480px] md:absolute md:bottom-0 md:right-0 md:top-[72px] md:mt-0 md:min-h-[540px] md:w-[58%] lg:w-[68%]`}
        >
        ...
        <a
          href="#proyectos"
          className={`${entranceClass} home-entrance--scroll absolute bottom-5 left-1/2 z-30 hidden -translate-x-1/2 items-center gap-4 text-[8px] font-bold uppercase tracking-[0.28em] text-muted-foreground md:flex`}
        >
